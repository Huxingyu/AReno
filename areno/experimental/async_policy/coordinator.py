"""Policy-version coordination for overlapped rollout and training."""
...
class PolicyPipelineCoordinator:
    ...
    engines. Policy synchronization is exclusive: it starts only after active
    rollouts and the current optimizer step have both reached a safe point, and
    it blocks new work until the transfer succeeds or fails.
    """
    ...
    def begin_rollout(self, *, timeout_s: float | None = None) -> int:
        ...
        with self._cond:
            self._wait_until(lambda: not self._sync_active, timeout_s, "begin_rollout")
    ...
    def begin_train(self, *, timeout_s: float | None = None) -> int:
        ...
        The lease may overlap rollout but never policy synchronization. The
        returned version is the policy state at the start of the update.
        """

        with self._cond:
            self._wait_until(lambda: not self._sync_active and not self._train_active, timeout_s, "begin_train")
    ...
    def begin_sync(self, *, timeout_s: float | None = None) -> PolicySyncPlan | None:
        ...
        with self._cond:
            self._wait_until(
                lambda: not self._sync_active and not self._train_active and self._active_rollouts == 0,
                timeout_s,
                "begin_sync",
            )
