...
2026-09-15 已在单机双 L4 上运行原生 LoRA 同步 / 异步短训练、GPU 重叠检查、保存重载、同步接收失败、worker 强退和同容器重启；详见 TODO 的 Modal 验收表。本轮没有出现新的功能性失败。接收端的主动异常和 worker exitcode=23 均是测试注入，验收要求它们传播并完成资源回收，不能计作新发现的产品 bug。

环境准备实际遇到 Modal API 连接超时：本机使用代理，而单独安装 `modal` 没有代理支持依赖。为独立工具 venv 安装 `modal[api-proxy-support]==1.5.5` 后，登录、CPU 准备和全部 GPU 任务均成功；项目依赖没有改动。GPU trace 原始数据较大，启动器改用快速压缩以减少 GPU 容器整理日志的等待。

首轮使用 toy reward（答案命中 + 小幅 token 多样性），只证明真实采样、非零梯度、更新和同步可运行。短训练及额度抵扣不构成吞吐、收敛或长期运行成本承诺。原型历史反例、CPU 压力矩阵、首轮 GPU 和后续补测分别保留其适用范围；当前验收状态以 TODO 为准。

**R4 补测发现并修复的问题**

| 问题 | 复现与修复 |
|---|---|
| 停止时原生 RPC 不响应取消 | CPU 用例在 producer 等 RPC 时请求停止，复现 worker close 未执行。bridge 现在绑定 stop event，RPC 等待退出后再统一回收；真实 GPU 通信超时和 SIGINT / SIGTERM 验收通过。 |
| 同步 SDK 丢失采样 seed | 两个干净 checkout 初始权重相同，指定相同 seed 却生成不同序列；CPU 翻译测试确认 seed=0 变成 None。现将 public seed 透传到 GPU；GRPO、GSPO 各自连续两次随机采样均完全相同。默认 seed=None 保持不变。 |
| CPU 通信测试误选 NCCL，失败后遗留进程 | 本机有单张 GPU 时，两项 Gloo 测试实际用 NCCL，触发 Duplicate GPU detected。测试现在对子进程隐藏 CUDA，并在失败后有界回收子进程与队列；185 项相关 CPU 回归通过。此项属于测试隔离问题。 |

首次随机轨迹回归的失败证据保留在 `r4-completion/regression-01/`。最终 `regression-02/` 使用从真实上游运行保存的固定训练输入，并比较 greedy rollout、materialization、loss / 梯度范数及最终权重；全部逐项一致。修复后的 seed 可复现性另行实测，没有放宽数值容差来修绿。

优化器续训是补全的交付能力：检查文件一致性后恢复 optimizer、训练 RNG 和更新计数；原子发布防止半成品替换最后成功 checkpoint。故障后的真实续训、固定输入的连续训练对照和正式命令行示例均已运行。独立质量实验改用 64 条训练题、32 条不重复评测题和最终数值答案奖励，结果与首轮 toy reward 分开记录。

**性能 / 质量实验留下的限制**

三种配置各完成三轮，lag=1 更新吞吐平均提高约 40%，但种子 43 的评测从训练前 21/32 降到 16/32，同步为 23/32。该轮 26/32 条回答达到 64 token 上限；8 道原来正确、后来错误的题全部在上限处截断，另有 3 道变正确。当前证据显示回答预算下的质量退化，不能据此确定 lag 或既定 loss 是唯一原因；未做更长预算和目标函数的因果对照。这个回退没有被“修绿”，质量稳定性仍未证明。

lag=0 三轮分别丢弃 54 / 54 / 55 个过期 batch，吞吐比同步低约 28%；严格限制 lag 不意味着更快。R4 的比较与记录任务已完成，完整数据见 TODO；小算术题集、短序列、55 次更新和 TP=1 / DP=1 的结论不能外推到长期收敛、多机或其他模型。
